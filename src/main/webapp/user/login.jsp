...
<div class="max-w-md w-full bg-white p-8 rounded-lg shadow-md border border-accent">
  ...
  <h2 class="text-2xl font-bold text-primary mb-6 text-center">MediCare Login</h2>
  <form method="post" action="${pageContext.request.contextPath}/user" class="space-y-4">
    ...
    <button type="submit"
            class="w-full bg-primary hover:bg-secondary text-white font-bold py-2 px-4 rounded transition">
      Login
    </button>

    <% if (request.getAttribute("error") != null) { %>
    <p class="text-red-500 text-center"><%= request.getAttribute("error") %>
    </p>
    <% } %>
  </form>

  <div class="mt-4 text-center">
    <p class="text-primary">Don't have an account? <a href="${pageContext.request.contextPath}/user?action=signup"
                                                      class="text-accent hover:text-secondary font-medium">Sign
      Up</a></p>
  </div>
</div>
</body>
